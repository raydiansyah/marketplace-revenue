...
const MARKETPLACE_COLORS: Record<MarketplaceId, string> = {
	shopee: "#EE4D2D",
	tokopedia: "#00AA5B",
	lazada: "#0F146D",
};
		...
		<tr className="border-b border-[var(--border-subtle)]">
			{[1, 2, 3, 4, 5, 6].map((i) => (
				<td key={i} className="px-4 py-3.5">
					<div
						className="h-3.5 rounded-md bg-[var(--surface-soft)] animate-pulse"
						style={{ width: `${40 + i * 10}%` }}
					/>
				</td>
			))}
...
function ConfirmDialog({
	...
	return (
		<div
			className="fixed inset-0 z-[100] flex items-center justify-center bg-black/35 backdrop-blur-sm p-4"
			role="dialog"
			aria-modal="true"
		>
			<div className="panel-card max-w-sm w-full p-6">
				<div className="flex items-start gap-3 mb-4">
					...
					<div>
						<p className="font-semibold text-[15px] text-[var(--foreground)] mb-1">
							Hapus file ini?
						</p>
						<p className="text-[13px] text-[var(--text-subtle)]">
							<strong className="text-[var(--foreground)]">{fileName}</strong>{" "}
							akan dihapus permanen dari Bank Data. Tindakan ini tidak bisa
							dibatalkan.
						</p>
					</div>
				</div>
				<div className="flex gap-2 justify-end">
					<button
						...
						onClick={onCancel}
						disabled={deleting}
						className="px-4 py-2 rounded-lg border border-[var(--border-subtle)] bg-[var(--surface)] text-[var(--foreground)] text-sm font-semibold disabled:opacity-60 transition-opacity"
					>
						Batal
					</button>
					<button
						...
						onClick={onConfirm}
						disabled={deleting}
						className="px-4 py-2 rounded-lg bg-[var(--negative)] text-white text-sm font-semibold disabled:opacity-60 inline-flex items-center gap-1.5 transition-opacity"
					>
...
function DataBankContent() {
	...
	// Get marketplace from URL query parameter (set by sub-menu)
	const urlMarketplace = searchParams.get(
		"marketplace",
	) as MarketplaceId | null;

	// If URL has marketplace, use it and hide the filter
	const isMarketplaceFromUrl = urlMarketplace !== null;
	...
	const [pendingDelete, setPendingDelete] =
		useState<MonthlyUploadRecord | null>(null);
	...
	const fetchRecords = useCallback(async () => {
		...
		try {
			...
			setTotalRecords(data.pagination?.total ?? data.uploads?.length ?? 0);
	...
	}, [filterMarketplace, filterStoreId, filterYear, filterMonth, currentPage, pageSize]);

	// Reset to page 1 when filters change
	useEffect(() => {
		setCurrentPage(1);
	}, [filterMarketplace, filterStoreId, filterYear, filterMonth, pageSize]);
		...
		<AuthAreaLayout>
			<div className="mx-auto max-w-[1400px] px-4 sm:px-6 py-6">
				...
				<div className="panel-card p-4 mb-6 flex flex-wrap gap-3 items-end">
					{/* Marketplace filter - hidden if accessed via sub-menu */}
					{!isMarketplaceFromUrl && (
						<div className="flex flex-col gap-1 min-w-[160px] flex-1 sm:flex-none">
							...
							<select
								...
								onChange={(e) =>
									setFilterMarketplace(e.target.value as MarketplaceId | "")
								}
					...
					{/* Show current marketplace badge when filtered via URL */}
					{isMarketplaceFromUrl && urlMarketplace && (
						<div className="flex flex-col gap-1 min-w-[160px] flex-1 sm:flex-none">
							<label className="text-xs font-semibold text-muted-foreground">
								Marketplace
							</label>
							<div className="field-input bg-[var(--surface-soft)] flex items-center gap-2 cursor-not-allowed">
								<span
									className="w-2.5 h-2.5 rounded-full"
									style={{
										backgroundColor: MARKETPLACE_COLORS[urlMarketplace],
									}}
								/>
								<span className="font-medium text-foreground">
									{MARKETPLACE_LABELS[urlMarketplace]}
								</span>
							</div>
						</div>
					)}
				...
				{!dataLoading && records.length === 0 && !dataError ? (
					<div className="panel-card text-center py-12 text-muted-foreground">
						...
						<button
							type="button"
							onClick={() => router.push("/upload")}
							className="mt-4 action-primary px-5 py-2 rounded-lg text-sm font-semibold"
						>
							Upload Sekarang
						</button>
					</div>
					...
					<div className="panel-card overflow-hidden">
										...
										records.map((record) => (
											...
											>
												...
												<td className="px-4 py-3 max-w-[200px] sm:max-w-[280px]">
													<span
														className="text-[13px] text-foreground font-medium block truncate"
												...
												<td className="px-4 py-3 whitespace-nowrap hidden sm:table-cell">
													<span className="text-[13px] text-muted-foreground">
														{MARKETPLACE_LABELS[record.marketplace] ??
															record.marketplace}
													</span>
												</td>
						...
						{!dataLoading && records.length > 0 && (
							<div className="px-4 py-2.5 border-t border-[var(--border-subtle)] flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 text-xs text-muted-foreground">
								<div>Menampilkan {records.length} dari {totalRecords} file</div>
								<div className="flex items-center gap-2">
									...
									<button
										...
										className="field-input py-1 px-3 text-xs disabled:opacity-50"
									>
										‹ Prev
									</button>
	...
	return (
		<Suspense
			fallback={
				<AuthAreaLayout>
					<div className="mx-auto max-w-[1400px] px-4 sm:px-6 py-6">
						<div className="panel-card p-8 text-center text-muted-foreground">
