...
import {
	listMonthlyUploads,
	upsertMonthlyUpload,
} from "@/lib/db/queries/monthlyUploads";
...
function parseFile(
	...
	switch (marketplace) {
		...
		case "tokopedia":
			return parseTokopediaFile(content);
		case "lazada":
			return parseLazadaFile(content);
	}
}
...
export async function GET(req: NextRequest) {
	try {
		...
		const pageSize = pageSizeRaw !== null ? Number(pageSizeRaw) : 20;
		...
		if (
			...
			!VALID_MARKETPLACES.includes(marketplace as MarketplaceId)
		) {
		...
		if (
			...
			(isNaN(periodYear) || periodYear < 2020 || periodYear > 2035)
		) {
			return NextResponse.json(
				{ error: "year tidak valid (2020-2035)" },
				{ status: 400 },
			);
		}
		if (
			periodMonth !== undefined &&
			(isNaN(periodMonth) || periodMonth < 1 || periodMonth > 12)
		) {
			return NextResponse.json(
				{ error: "month tidak valid (1-12)" },
				{ status: 400 },
			);
		}
		if (
		...
		const records = await listMonthlyUploads({
			userId: session.sub,
			marketplace: marketplace as MarketplaceId | undefined,
			storeId,
			periodYear,
			periodMonth,
			fileType,
		...
		return NextResponse.json(
			{
				...
				pagination: {
					...
					total: records.total,
					totalPages: Math.ceil(records.total / safePageSize),
				},
			},
			{
				headers: {
...
export async function POST(req: NextRequest) {
	try {
		...
		const replaceRaw = formData.get("replace");
		...
		if (
			!marketplaceRaw ||
			!VALID_MARKETPLACES.includes(marketplaceRaw as MarketplaceId)
		) {
			return NextResponse.json(
				{
					error:
						"marketplace wajib diisi dan harus shopee, tokopedia, atau lazada",
				},
				{ status: 400 },
			);
		}
		...
		if (isNaN(periodYear) || periodYear < 2020 || periodYear > 2035) {
			return NextResponse.json(
				{ error: "periodYear tidak valid (2020-2035)" },
				{ status: 400 },
			);
		}
		if (isNaN(periodMonth) || periodMonth < 1 || periodMonth > 12) {
			return NextResponse.json(
				{ error: "periodMonth tidak valid (1-12)" },
				{ status: 400 },
			);
		}
		...
		if (file.size > MAX_FILE_SIZE) {
			return NextResponse.json(
				{ error: "Ukuran file melebihi batas 50 MB" },
				{ status: 413 },
			);
		}
		...
		// ── 8. Persist (replace or insert) ────────────────────────────────
		const id = randomUUID();
		const result = await upsertMonthlyUpload(
			{
				id,
				userId: session.sub,
		...
		if (result === "duplicate") {
			return NextResponse.json(
				{
					error: `File "${file.name}" sudah diupload untuk periode ${periodYear}-${String(periodMonth).padStart(2, "0")} di marketplace ini`,
				},
				{ status: 409 },
			);
		}
