...
import { ChevronDown, Plus, Check, Loader2, AlertCircle, X } from "lucide-react";
...
export default function StorePicker({
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setCreateError(err instanceof Error ? err.message : "Gagal membuat toko.");
      setCreateState("idle");
    }
  }
  ...
  const buttonLabel = fetchState === "loading"
    ? "Memuat toko..."
    : selectedStore
    ...
    <div ref={containerRef} style={{ position: "relative" }}>
      ...
      <button
        ...
        style={{
          ...
          fontSize: "0.875rem",
          cursor: disabled || fetchState === "loading" ? "not-allowed" : "pointer",
      ...
      >
        <span style={{ display: "flex", alignItems: "center", gap: "0.4rem", minWidth: 0 }}>
          {fetchState === "loading" && (
            <Loader2 style={{ width: "0.875rem", height: "0.875rem", flexShrink: 0, animation: "spin 1s linear infinite" }} />
          )}
      ...
      {fetchState === "error" && (
        <p style={{ marginTop: "0.25rem", fontSize: "0.75rem", color: "var(--negative)", display: "flex", alignItems: "center", gap: "0.25rem" }}>
          <AlertCircle style={{ width: "0.75rem", height: "0.75rem" }} />
          Gagal memuat toko.{" "}
          <button
            type="button"
            onClick={() => void fetchStores()}
            style={{ textDecoration: "underline", background: "none", border: "none", cursor: "pointer", color: "var(--negative)", fontSize: "inherit" }}
          >
            Coba lagi
          </button>
        </p>
        ...
        >
          ...
          <ul style={{ listStyle: "none", margin: 0, padding: "0.375rem" }}>
            {stores.length === 0 && fetchState !== "loading" && (
              <li style={{ padding: "0.5rem 0.75rem", fontSize: "0.8125rem", color: "var(--text-subtle)" }}>
            ...
            {stores.map((store) => (
              <li key={store.id}>
                ...
                >
                  <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{store.storeName}</span>
                  {store.id === value && (
                    <Check style={{ width: "0.875rem", height: "0.875rem", flexShrink: 0, color: "var(--positive)" }} />
                  )}
                </button>
              </li>
            ))}
          ...
          {/* Divider */}
          <div style={{ height: "1px", background: "var(--border-subtle)", margin: "0 0.375rem" }} />
          ...
          <div style={{ padding: "0.375rem" }}>
            {!showCreate ? (
              ...
              <div style={{ padding: "0.25rem 0.375rem", display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                <div style={{ display: "flex", gap: "0.375rem" }}>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      fontSize: "0.8125rem",
                      fontWeight: 600,
                      cursor: !newStoreName.trim() || createState === "loading" ? "not-allowed" : "pointer",
                      opacity: !newStoreName.trim() || createState === "loading" ? 0.6 : 1,
                      display: "flex",
                      alignItems: "center",
                      gap: "0.25rem",
                    }}
                  ...
                  <button
                    type="button"
                    onClick={() => { setShowCreate(false); setNewStoreName(""); setCreateError(null); }}
                    style={{
                      padding: "0.4rem",
                      background: "none",
                      border: "1px solid var(--border-subtle)",
                ...
                {createError && (
                  <p style={{ fontSize: "0.75rem", color: "var(--negative)", display: "flex", alignItems: "center", gap: "0.25rem", margin: 0 }}>
